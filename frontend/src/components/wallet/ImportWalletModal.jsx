import React, { useState } from 'react';
import { FaWallet, FaTimes, FaUpload, FaExclamationTriangle } from 'react-icons/fa';
import { useWallet } from '../../hooks/useWallet';
import { useUI } from '../../hooks/useUI';

const ImportWalletModal = ({ isOpen, onClose }) => {
  const { importWallet, loading } = useWallet();
  const { blockchainApi } = useUI();
  
  const [walletData, setWalletData] = useState({
    name: '',
    blockchain: 'solana',
    address: '',
    privateKey: '',
    key: ''
  });
  
  const [showWarning, setShowWarning] = useState(false);
  
  const handleChange = (e) => {
    const { name, value } = e.target;
    setWalletData(prev => ({ ...prev, [name]: value }));
  };
  
  const handleSubmit = async (e) => {
    e.preventDefault();
    
    // Show warning first time
    if (!showWarning) {
      setShowWarning(true);
      return;
    }
    
    try {
      await importWallet(walletData);
      
      // Reset form and close modal
      setWalletData({
        name: '',
        blockchain: 'solana',
        address: '',
        privateKey: '',
        key: ''
      });
      
      setShowWarning(false);
      onClose();
    } catch (error) {
      // Error is handled in context
    }
  };
  
  // Determine blockchain options based on selected API
  const getBlockchainOptions = () => {
    if (blockchainApi.includes('tatum')) {
      return [
        { value: 'solana', label: 'Solana' },
        { value: 'ethereum', label: 'Ethereum' },
        { value: 'bitcoin', label: 'Bitcoin' },
        { value: 'binance', label: 'Binance Smart Chain' },
        { value: 'polygon', label: 'Polygon' }
      ];
    } else if (blockchainApi.includes('web3')) {
      return [
        { value: 'ethereum', label: 'Ethereum' }
      ];
    } else if (blockchainApi.includes('solana')) {
      return [
        { value: 'solana', label: 'Solana' }
      ];
    }
    
    return [
      { value: 'solana', label: 'Solana' }
    ];
  };
  
  if (!isOpen) return null;
  
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md">
        <div className="flex justify-between items-center p-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white flex items-center">
            <FaWallet className="mr-2" /> Import Existing Wallet
          </h2>
          <button
            onClick={onClose}
            className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
          >
            <FaTimes />
          </button>
        </div>
        
        <form onSubmit={handleSubmit} className="p-4">
          {showWarning ? (
            <div className="mb-6 p-4 bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 rounded-md">
              <div className="flex items-start">
                <FaExclamationTriangle className="text-yellow-500 mt-0.5 mr-3 flex-shrink-0" />
                <div>
                  <h3 className="text-sm font-medium text-yellow-800 dark:text-yellow-300">
                    Security Warning
                  </h3>
                  <p className="mt-1 text-sm text-yellow-700 dark:text-yellow-400">
                    Your private key will be encrypted and stored securely. Never share your private key with anyone.
                  </p>
                  <p className="mt-2 text-sm text-yellow-700 dark:text-yellow-400">
                    Are you sure you want to proceed with importing this wallet?
                  </p>
                  
                  <div className="mt-4 flex gap-2">
                    <button
                      type="button"
                      onClick={() => setShowWarning(false)}
                      className="px-3 py-1.5 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white rounded-md text-sm"
                    >
                      Go Back
                    </button>
                    
                    <button
                      type="submit"
                      className="px-3 py-1.5 bg-yellow-500 text-white rounded-md text-sm"
                    >
                      Yes, Import Wallet
                    </button>
                  </div>
                </div>
              </div>
            </div>
          ) : (
            <>
              <div className="mb-4">
                <label htmlFor="name" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Wallet Name
                </label>
                <input
                  type="text"
                  id="name"
                  name="name"
                  value={walletData.name}
                  onChange={handleChange}
                  required
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                  placeholder="My Imported Wallet"
                />
              </div>
              
              <div className="mb-4">
                <label htmlFor="blockchain" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Blockchain
                </label>
                <select
                  id="blockchain"
                  name="blockchain"
                  value={walletData.blockchain}
                  onChange={handleChange}
                  required
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                >
                  {getBlockchainOptions().map(option => (
                    <option key={option.value} value={option.value}>
                      {option.label}
                    </option>
                  ))}
                </select>
              </div>
              
              <div className="mb-4">
                <label htmlFor="address" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Wallet Address
                </label>
                <input
                  type="text"
                  id="address"
                  name="address"
                  value={walletData.address}
                  onChange={handleChange}
                  required
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                  placeholder="Enter wallet address"
                />
              </div>
              
              <div className="mb-4">
                <label htmlFor="privateKey" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Private Key
                </label>
                <input
                  type="password"
                  id="privateKey"
                  name="privateKey"
                  value={walletData.privateKey}
                  onChange={handleChange}
                  required
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                  placeholder="Enter private key"
                />
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Your private key will be encrypted and stored securely
                </p>
              </div>
              
              <div className="mb-4">
                <label htmlFor="key" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Your Access Key
                </label>
                <input
                  type="password"
                  id="key"
                  name="key"
                  value={walletData.key}
                  onChange={handleChange}
                  required
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                  placeholder="Enter your 16-character key"
                  maxLength={16}
                  minLength={16}
                />
              </div>
              
              <div className="mt-6 flex justify-end gap-2">
                <button
                  type="button"
                  onClick={onClose}
                  className="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white rounded-md hover:bg-gray-300 dark:hover:bg-gray-600"
                >
                  Cancel
                </button>
                
                <button
                  type="submit"
                  disabled={loading}
                  className={`px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-blue-400 disabled:cursor-not-allowed flex items-center`}
                >
                  {loading ? (
                    <>
                      <svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                      </svg>
                      Importing...
                    </>
                  ) : (
                    <>
                      <FaUpload className="mr-2" /> Import Wallet
                    </>
                  )}
                </button>
              </div>
            </>
          )}
        </form>
      </div>
    </div>
  );
};

export default ImportWalletModal;